...
#define JNUM 3 //judge point
#define DETECTLEVEL 3 // detect LEVEL
using namespace std;
...
AIPlayer5::AIPlayer5(ChessType Color)
{
    ...
    evaBoard["aa___"]=chessModel["SleepTwo"];
    ...
    evaBoard["__aa__"]=chessModel["LiveTwo"]+1;
    ...
    evaBoard["aaa__"]=chessModel["SleepThree"]+1;

    evaBoard["_aa_a_"]=chessModel["LiveThree"]-1;
    evaBoard["_a_aa_"]=chessModel["LiveThree"]-1;

    evaBoard["_aaa_"]=chessModel["LiveThree"]+1;


    evaBoard["a_aaa"]=chessModel["SleepFour"];
    evaBoard["aaa_a"]=chessModel["SleepFour"];
    evaBoard["_aaaa"]=chessModel["SleepFour"]+1;
    evaBoard["aaaa_"]=chessModel["SleepFour"]+1;
    evaBoard["aa_aa"]=chessModel["SleepFour"];


    evaBoard["_aaaa_"]=chessModel["LiveFour"];

    evaBoard["aaaaa"]=chessModel["Five"];
...
void AIPlayer5::playChess()
{
    ...
    for(auto &item : Mnn)
    {
        ...
        copyArray(ChessBoard::getInstance()->myChessBoard,cloneChessBoard);
        createTree(item,cloneChessBoard,DETECTLEVEL,false);

        float a = -chessModel["Five"];
        float b = chessModel["Five"];
        item.value += AlphaBeta(item,DETECTLEVEL,false,a,b);
...
vector<MiniMaxNode2> AIPlayer5::GetVector(char chessboard[][15],ChessType myChessColor,bool myself)
{
    ...
    {
        float ans = 0;
        ans += getLineScore(chessboard,nodeVector[i].pos,pair<short,short>(1,0),chessColor)*1.2;
        ans += getLineScore(chessboard,nodeVector[i].pos,pair<short,short>(0,1),chessColor)*1.2;
        ans += getLineScore(chessboard,nodeVector[i].pos,pair<short,short>(1,1),chessColor)*1.2;
        ans += getLineScore(chessboard,nodeVector[i].pos,pair<short,short>(1,-1),chessColor)*1.2;
        ans += getLineScore(chessboard,nodeVector[i].pos,pair<short,short>(1,0),ChessType(-chessColor+3));
        ans += getLineScore(chessboard,nodeVector[i].pos,pair<short,short>(0,1),ChessType(-chessColor+3));
        ans += getLineScore(chessboard,nodeVector[i].pos,pair<short,short>(1,1),ChessType(-chessColor+3));
        ans += getLineScore(chessboard,nodeVector[i].pos,pair<short,short>(1,-1),ChessType(-chessColor+3));
        nodeVector[i].point = ans;
    }
    sort(nodeVector.begin(),nodeVector.end(),_cmp);
    return nodeVector;
}
